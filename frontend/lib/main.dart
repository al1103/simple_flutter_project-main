...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> checkServer() async {
    final url = Uri.parse('http://localhost:8080/api/v1/check');
    try {
  ...
  Future<void> echoMessage(String message) async {
    final url = Uri.parse('http://localhost:8080/api/v1/echo/$message');
    try {
      final response = await http.get(url).timeout(const Duration(seconds: 10));
      if (response.statusCode == 200) {
        final data = json.decode(response.body);
        setState(() {
          responseMessage = 'Phản hồi từ máy chủ: ${data['message']}';
        });
      } else {
        setState(() {
          responseMessage = 'Máy chủ không phản hồi đúng';
        ...
        child: Column(
          children: [
            ...
            FilledButton(
              ...
              child: const Text('Kiểm tra máy chủ'),
            ),
            const SizedBox(height: 10),
            FilledButton(
